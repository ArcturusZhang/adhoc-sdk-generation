...
1. **`<ClientClassName>.cs`** — a single thin client class with one method per operation (sync + async pair). Mgmt-style:
   ...
   - **Validate every reference parameter** at the top of every public method and constructor. For each non-`CancellationToken`, non-nullable-value-type, non-optional reference parameter, emit `if (paramName == null) throw new ArgumentNullException(nameof(paramName));` before any other logic. This includes `subscriptionId`, `resourceGroupName`, path parameters, body parameters, `endpoint`, and `credential`. Without these, a `null` argument crashes deep inside `RequestUriBuilder.AppendPath` with an unhelpful `NullReferenceException`.
...
4. **`<ClientStem>RestClient.cs`** — internal helper that owns request construction (`CreateXxxRequest(...)` returning `HttpMessage`), kept separate from the public client for readability. The **`<ClientStem>`** is `<ClientClassName>` with one trailing `Client` removed if present (so `AvailabilitySetsClient` → `AvailabilitySets` → `AvailabilitySetsRestClient.cs`; if `<ClientClassName>` is `Foo`, stem stays `Foo` → `FooRestClient.cs`). Avoid producing names like `AvailabilitySetsClientRestClient` with double "Client". The internal class name inside the file matches the filename.
...
// Source swagger: <exact swaggerPath the user provided>
// Operations: <comma-separated operationIds>
```

The `Source swagger:` value must be the **exact** string the user passed as `swaggerPath` — do not abbreviate, summarize, or invent a path. Many swagger files share the same filename (e.g., `ComputeRP.json` exists under multiple service folders and api-versions); only the full path is unambiguous.

For a model that came in via a `$ref` to a sibling file (e.g., common-types), add a second line on that file only with the resolved relative path of the `$ref`:

```csharp
// Resolved via $ref: ../../../../../../common-types/resource-management/v3/types.json#/definitions/TrackedResource
```

Use the literal `$ref` string from the swagger; do not abbreviate it.
